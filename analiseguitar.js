...
var cor1 = [], cor2 = [], cor3 = [], cor4 = [], cor5 = [], cor6 = [], cor7 = [], cor8 = [], cor9 = [], cor10 = [], cor11 = [], cor12 = [], cor13 = [], cor14 = [], cor15 = [], cor16 = []
var song;
var button;
//let playing = false;
let diameter = 300;
...
//Preload//
function preload() {
  songguitar = loadSound("guitarwithecho.wav"); // Ficheiro áudio
  pitchguitar = loadTable('pitchguitar.txt', 'tsv');  // Pitch 
  analiseguitar = loadTable('analiseguitar.txt', 'csv'); // Análise Tempo / Cores
}
...
function setup() {
  ...
  notes.push(parseInt(pitchguitar.getString(0,0))+(12*5))
  notes.push(parseInt(pitchguitar.getString(1,0))+(12*5))
  notes.push(parseInt(pitchguitar.getString(2,0))+(12*5))
  notes.push(parseInt(pitchguitar.getString(3,0))+(12*5))
  notes.push(parseInt(pitchguitar.getString(4,0))+(12*5))
  notes.push(parseInt(pitchguitar.getString(5,0))+(12*5))
  notes.push(parseInt(pitchguitar.getString(6,0))+(12*5))
  //print(notes)
  for (let c = 1; c < analiseguitar.getColumnCount(); c++) {
    cor1.push(analiseguitar.getString(0,c)*255)
    cor2.push(analiseguitar.getString(1,c)*255)
    cor3.push(analiseguitar.getString(2,c)*255)
    cor4.push(analiseguitar.getString(3,c)*255)
    cor5.push(analiseguitar.getString(4,c)*255)
    cor6.push(analiseguitar.getString(5,c)*255)
    cor7.push(analiseguitar.getString(6,c)*255)
    cor8.push(analiseguitar.getString(7,c)*255)
    cor9.push(analiseguitar.getString(8,c)*255)
    cor10.push(analiseguitar.getString(9,c)*255)
    cor11.push(analiseguitar.getString(10,c)*255)
    cor12.push(analiseguitar.getString(11,c)*255)
    cor13.push(analiseguitar.getString(12,c)*255)
    cor14.push(analiseguitar.getString(13,c)*255)
    //cor15.push(analiseguitar.getString(14,c)*255)
    //cor16.push(analiseguitar.getString(15,c)*255)
...
function pieChart(diameter) {
  ...
  fill(cor1[i])
    ...
    if (Math.floor(audio.currentTime % 60) >= analiseguitar.getString(10,0)) { //0 que é 1 
      fill(cor11[i])
    }
    if (Math.floor(audio.currentTime % 60)  >= analiseguitar.getString(11,0)) {//1 que é 2
      fill(cor12[i])
    }
    if (Math.floor(audio.currentTime % 60) >= analiseguitar.getString(12,0)) { //2 que é 3
      fill(cor13[i])
    }
    if (Math.floor(audio.currentTime % 60) >= analiseguitar.getString(13,0)) { //2 que é 3
      fill(cor14[i])
    }
    /*if (Math.floor(audio.currentTime % 60) >= analiseguitar.getString(14,0)) { //2 que é 3
      fill(cor15[i])
    }
    
    //if (Math.floor(audio.currentTime % 60) >= analiseguitar.getString(15,0)) { //2 que é 3
    //  fill(cor16[i])
    //}
